  ...
  return (
    <div>
      <div className="flex justify-between">
        ...
        <GitHubIcon
          url="https://github.com/vbe0201/kobold"
          size={16}
          className="mr-2"
        />
      </div>
      {tabs.length != 0 ? (
        <TabView {...{ tabs, closeTab, createTab }} />
      ) : (
        <NoTabs createTab={() => createTab("New Tab")} />
      )}
    </div>
  );
};
...
const NoTabs: FC<{ createTab: () => void }> = ({ createTab }) => {
  return (
    <div className="flex justify-center align-middle flex-col text-center gap-4">
      <p className="text-xl pt-5">You have no open tabs.</p>
      <Button className="m-auto" onClick={createTab}>
        Create a new Tab
      </Button>
    </div>
  );
