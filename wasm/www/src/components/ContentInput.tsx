import { FileInput, Tabs, Textarea } from "@mantine/core";
...
const ContentInput: FC<ContentInputProps> = ({ setContent }) => {
  ...
  return <Tabs defaultValue='file' value={activeTab} onTabChange={setActiveTab}>
    <Tabs.List>
      <Tabs.Tab value='file'>
        File
      </Tabs.Tab>
    ...
    <Tabs.Panel value='file'>
      <FileInput placeholder='Choose a file' label='File' withAsterisk value={file} onChange={setFile} />
    </Tabs.Panel>
    <Tabs.Panel value='hex'>
      <Textarea className="h-96" placeholder='Hex content' label='Content' withAsterisk value={hex} onChange={(event) => setHex(event.target.value)} />
    </Tabs.Panel>
  </Tabs>
}
